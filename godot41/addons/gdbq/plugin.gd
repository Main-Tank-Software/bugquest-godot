...
func check_for_update():
	var current_version = get_local_plugin_version()
	if current_version == "":
		push_error("Could not determine local plugin version")
		return
	...
	http.request_completed.connect(func(result, response_code, headers, body):
		if response_code != 200:
			print("GitHub version check failed with code:", response_code)
			return
...
func show_update_notification(latest_version: String):
	var popup := PopupPanel.new()
	popup.name = "PluginUpdatePopup"

	# VBox container for layout
	var vbox := VBoxContainer.new()
	vbox.custom_minimum_size = Vector2(420, 120)  # Reduced height
	vbox.add_theme_constant_override("separation", 8)  # Tighter vertical spacing
	# Do NOT expand vertically
	vbox.size_flags_vertical = Control.SIZE_FILL

	# Label
	var label := Label.new()
	label.text = "New BugQuest version (%s) is available!" % latest_version
	...
	open_button.text = "View on GitHub"
	open_button.pressed.connect(func():
		OS.shell_open("https://github.com/Main-Tank-Software/bugquest-godot/releases/latest")
		popup.queue_free()
	)
	...
	call_deferred("_popup_dialog", popup)

func _popup_dialog(popup: PopupPanel):
